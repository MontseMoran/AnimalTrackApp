...
function AnimalFormScreen() {
  ...
  const scheduleNotification = async (title, body, dateTime) => {
    try {
      ...
      console.log('🔔 Notificación programada:', title, dateTime);
    } catch (error) {
  ...
  const handleAddTreatment = () => {
    ...
    if (editingTreatmentId) {
      setTreatments(treatments.map((t) => (t.id === editingTreatmentId ? newTreatment : t)));
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>{editingAnimal ? 'Editar Animal 🐾' : 'Añadir Animal 🐾'}</Text>

      <TouchableOpacity onPress={pickImage} style={styles.photoContainer}>
        {photo ? <Image source={{ uri: photo }} style={styles.photo} /> : <Text style={styles.photoPlaceholder}>+ Añadir foto</Text>}
      </TouchableOpacity>

      <View style={styles.photoButtons}>
        <TouchableOpacity style={styles.smallButton} onPress={pickImage}><Text style={styles.smallButtonText}>Galería</Text></TouchableOpacity>
        <TouchableOpacity style={styles.smallButton} onPress={takePhoto}><Text style={styles.smallButtonText}>Cámara</Text></TouchableOpacity>
      </View>
      ...
      <TextInput style={styles.input} placeholder="Fecha de nacimiento" value={birthDate} onChangeText={setBirthDate} />

      <View style={styles.switchContainer}><Text>Tiene chip</Text><Switch value={hasChip} onValueChange={setHasChip} /></View>
      {hasChip && <TextInput style={styles.input} placeholder="Número de chip" value={chipNumber} onChangeText={setChipNumber} />}

      <View style={styles.switchContainer}><Text>Esterilizado</Text><Switch value={isSterilized} onValueChange={setIsSterilized} /></View>
      {isSterilized && <TextInput style={styles.input} placeholder="Fecha de esterilización" value={sterilizationDate} onChangeText={setSterilizationDate} />}

      {/* ---------------- TRATAMIENTOS Y RECORDATORIOS ---------------- */}
      <Text style={styles.sectionTitle}>Tratamientos / recordatorios</Text>
      <TextInput style={styles.input} placeholder="Tipo (Vacuna, Medicación...)" value={tempType} onChangeText={setTempType} />
      <TextInput style={styles.input} placeholder="Nombre del tratamiento" value={tempName} onChangeText={setTempName} />
      <TextInput style={styles.input} placeholder="Fecha o frecuencia" value={tempDate} onChangeText={setTempDate} />
      <TextInput style={[styles.input, { height: 60 }]} placeholder="Notas o dosis" value={tempNotes} onChangeText={setTempNotes} multiline />

      <View style={styles.switchContainer}><Text>¿Activar recordatorio?</Text><Switch value={reminderEnabled} onValueChange={setReminderEnabled} /></View>
      ...
      {reminderEnabled && (
        <View>
          ...
          <View style={styles.input}>
            <Picker selectedValue={reminderRepeat} onValueChange={(v) => setReminderRepeat(v)}>
          ...
          {reminderDate && (
            <Text style={styles.summary}>
              📅 Próximo recordatorio: {reminderDate.toLocaleDateString()} a las {reminderDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              {'\n'}🔁 {reminderRepeat === 'once' ? 'Una sola vez' : reminderRepeat === 'customHours' ? `Cada ${customHours} horas` : `Repetición: ${reminderRepeat}`}
            </Text>
          )}
      ...
      <TouchableOpacity style={styles.smallButton} onPress={handleAddTreatment}>
          ...
          {treatments.map((t) => (
            <View key={t.id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
              <View style={{ flex: 1 }}>
                ...
                {t.notes ? <Text style={{ color: colors.textSecondary, fontSize: 13 }}>→ {t.notes}</Text> : null}
              </View>
              <TouchableOpacity onPress={() => handleEditTreatment(t)} style={{ marginHorizontal: 6 }}>
                <Text>✏️</Text>
              </TouchableOpacity>
      ...
      <TextInput style={[styles.input, { height: 80 }]} placeholder="Notas generales del animal" value={notes} onChangeText={setNotes} multiline />

      <View style={styles.buttonContainer}>
        <TouchableOpacity style={styles.button} onPress={handleSave}>
          <Text style={styles.buttonText}>{editingAnimal ? 'Guardar cambios' : 'Guardar'}</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
